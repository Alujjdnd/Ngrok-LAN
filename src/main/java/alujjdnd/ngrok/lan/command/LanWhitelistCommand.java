...
public class LanWhitelistCommand {
	...
	public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
		dispatcher.register((CommandManager.literal("whitelist").requires((source) ->
			source.hasPermissionLevel(3)
		)).then(CommandManager.literal("on").executes((context) ->
			executeOn(context.getSource())
		)).then(CommandManager.literal("off").executes((context) ->
			executeOff(context.getSource())
			...
			return CommandSource.suggestMatching(playerManager.getPlayerList().stream().filter((player) ->
				!playerManager.getWhitelist().isAllowed(player.getGameProfile())
			).map((player) ->
				player.getGameProfile().getName()
			), builder);
		}).executes((context) ->
			executeAdd(context.getSource(), GameProfileArgumentType.getProfileArgument(context, "targets"))
			...
			CommandSource.suggestMatching(context.getSource().getServer().getPlayerManager().getWhitelistedNames(), builder)
		).executes((context) ->
			executeRemove(context.getSource(), GameProfileArgumentType.getProfileArgument(context, "targets"))
